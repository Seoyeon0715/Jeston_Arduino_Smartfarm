...
# **Sensor Data Query and Gradio UI Implementation Project**

This code processes **Arduino sensor data** in real-time or from a file and provides a user interface using **the OpenAI API** and **Gradio UI**.
...
### **Basic Setup and Importing Libraries**
...
def moisture_sensor_info(mode='real-time', file_path=None, port='/dev/ttyUSB0', baudrate=9600):
    ...
    Parameters:
    - mode (str): 'real-time' or 'file'. Determines real-time or file-based data processing.
    - file_path (str): Path to the CSV file (used in 'file' mode).
    - port (str): Serial port for the sensor (used in 'real-time' mode).
...
## **Sensor Function Definition (sensor_functions)**

`sensor_functions` is a JSON structure defined for OpenAI API calls. Each function is designed to retrieve specific sensor data either in real-time or from a stored file.
...
Each function is composed of the following properties:

1. **`name`**: The name of the function (e.g., `moisture_sensor_info`)
2. **`description`**: Description of the function's role and purpose
3. **`parameters`**:
   - **`mode`**: Choose between `real-time` or `file` mode
